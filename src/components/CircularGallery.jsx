...
class Media {
  ...
  createTitle() {
    // Disabled text titles below 3D project cards
  }
...
class App {
  ...
  onTouchMove(e) {
    ...
    if (this.container && this.medias && this.medias.length && this.viewport) {
      ...
      const viewportX = clickRatio * (this.viewport.width / 2);
      ...
      if (closestMedia && minDistance < (this.viewport.width / 3)) {
        const projectIdx = closestMedia.index % (this.mediasImages.length / 2);
        const cardXPercent = Math.max(15, Math.min(85, ((closestMedia.plane.position.x / (this.viewport.width / 2)) + 1) / 2 * 100));
        window.__hoveredProjectIndex = projectIdx;

        if (this.onHoverItem) {
          this.onHoverItem(projectIdx, { xPercent: cardXPercent, yPercent: 50 });
        }
      } else if (this.onHoverItem) {
        this.onHoverItem(null, null);
      }
    }

    if (!this.isDown) return;
    if (Math.hypot(x - this.start, y - this.startY) > 8) {
  ...
  onTouchUp(e) {
    if (this.isClickCandidate && this.onItemClick && this.medias && this.medias.length && this.container) {
      const rect = this.container.getBoundingClientRect();
      const clientX = e && e.changedTouches ? e.changedTouches[0].clientX : (e ? e.clientX : this.start);
      const clickRatio = ((clientX - rect.left) / rect.width) * 2 - 1;
      const viewportX = clickRatio * (this.viewport.width / 2);
      ...
      this.medias.forEach((media) => {
        ...
        if (dist < minDistance) {
          minDistance = dist;
          closestIndex = media.index % (this.mediasImages.length / 2);
        }
      });
      this.onItemClick(closestIndex);
    } else {
      this.onCheck();
    }
    this.isDown = false;
