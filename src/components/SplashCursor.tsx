...
export const SplashCursor: React.FC<SplashCursorProps> = ({
  ...
  useEffect(() => {
    ...
    class Material {
      ...
      setKeywords(keywords: string[]) {
        ...
        if (program == null) {
          ...
          if (fragmentShader) {
            const p = createProgram(this.vertexShader, fragmentShader);
            if (p) {
              program = p;
        ...
        if (program === this.activeProgram || !program) return;
    ...
    function updateFrame() {
      ...
      // Only perform heavy WebGL fluid calculations if pointer moved recently (< 60 frames / 1sec)
      if (idleCounter < 60) {
